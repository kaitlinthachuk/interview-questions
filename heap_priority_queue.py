class PriorityQueue:
    ...
    def heapify_ascending(self, index):
        ...
        while (
            index > 0
            and self._queue[parent_index]["priority"] > self._queue[index]["priority"]
        ):
            # Swap with parent
    ...
    def heapify_descending(self, index):
        ...
        if (
            left_child < len(self._queue)
            and self._queue[left_child]["priority"] < self._queue[smallest]["priority"]
        ):
            smallest = left_child

        if (
            right_child < len(self._queue)
            and self._queue[right_child]["priority"] < self._queue[smallest]["priority"]
        ):
        ...
        if (
            right_child < len(self._queue)
            and self._queue[right_child]["priority"]
            == self._queue[smallest]["priority"]
        ):
